...
$__Context->oDB = &DB::getInstance();
 ...
 ?>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','_setting.html') ?>
<!--#Meta:modules/board/skins/KSO_application_form_02/js/jquery.cookie.js--><?php $__tmp=array('modules/board/skins/KSO_application_form_02/js/jquery.cookie.js','body','','');Context::loadFile($__tmp);unset($__tmp); ?>
<script>//<![CDATA[
...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','video.html');
} ?>
<?php if($__Context->module_info->default_style=='videolist'){;
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','videolist.html');
} ?>
...
<?php if($__Context->oDocument->isExists() && $__Context->module_info->default_style != 'blog'){ ?>
    <?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','view_document.html') ?>
...
<?php } ?>
<?php if($__Context->module_info->use_category=='Y'){ ?><ul class="nav pull-left <?php if($__Context->module_info->display_categoryset_button == 'nonav'){ ?>hidden<?php }else{ ?>nav-pills<?php } ?>">
    <li<?php if(!$__Context->category){ ?> class="active"<?php } ?>>
        <a href="<?php echo getUrl('category','','page','','document_srl','') ?>" title="<?php echo $__Context->lang->document_count ?> '<?php echo number_format($__Context->total_count) ?>'" class="colorBgHover"><?php echo $__Context->lang->total ?> <?php if($__Context->module_info->display_categoryset_badge=='Y'){ ?><span class="badge"><?php echo $__Context->result->total ?></span><?php } ?></a>
    </li>
    <?php if($__Context->cate_list&&count($__Context->cate_list))foreach($__Context->cate_list as $__Context->key=>$__Context->val){ ?><li class="hidden-sm hidden-xs <?php if($__Context->category==$__Context->val->category_srl){ ?>active<?php } ?>" >
        <a href="<?php echo getUrl(category,$__Context->val->category_srl,'document_srl','', 'page', '') ?>" class="colorBgHover"><?php echo $__Context->val->title ?> <?php if($__Context->module_info->display_categoryset_badge=='Y'){ ?><span class="badge"><?php echo $__Context->val->document_count ?></span><?php } ?></a>
    </li><?php } ?>
    <li class="dropdown hidden-lg hidden-md"<?php if($__Context->category){ ?> class="active"<?php } ?>>
      ...
      <ul class="dropdown-menu" role="menu">
        <?php if($__Context->cate_list&&count($__Context->cate_list))foreach($__Context->cate_list as $__Context->key=>$__Context->val){ ?><li<?php if($__Context->category==$__Context->val->category_srl){ ?> class="active"<?php } ?>><a href="<?php echo getUrl(category,$__Context->val->category_srl,'document_srl','', 'page', '') ?>"><?php echo $__Context->val->title ?> <?php if($__Context->module_info->display_categoryset_badge=='Y'){ ?><span class="badge pull-right"><?php echo $__Context->val->document_count ?></span><?php } ?></a></li><?php } ?>
      </ul>
    </li>
</ul><?php } ?>
...
<div class="nav-pills-cover">
    <?php if($__Context->module_info->use_category=='Y'){ ?><ul class="nav nav-pills">
...
<?php if($__Context->module_info->default_style == 'webzine'){ ?>
    <?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','_style.webzine.html') ?>
<?php }elseif($__Context->module_info->default_style == 'gallery'){ ?>
    <?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','_style.gallery.html') ?>
<?php }elseif($__Context->module_info->default_style == 'card'){ ?>
    <?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','_style.card.html') ?>
...
<?php if($__Context->document_list&&count($__Context->document_list))foreach($__Context->document_list as $__Context->no=>$__Context->oDocument){ ?>
	<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','_style.blog.html') ?>
<?php } ?>
<?php }else{ ?>	
    <?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO_application_form_02','_style.list.html') ?>
<?php } ?>
<div>
