...
<?php if($__Context->document_list || $__Context->notice_list){ ?><table class="table table-hover">
    <thead class="infoN custom_tbody">
        <tr class="active">
            ...
            <?php if($__Context->grant->manager){ ?><td class="bold width30 mobileCheck"></td><?php } ?>
        </tr>
    </thead>
	<tbody class="custom_tbody">
		
		<?php if($__Context->notice_list&&count($__Context->notice_list))foreach($__Context->notice_list as $__Context->no=>$__Context->document){ ?><tr class="<?php if($__Context->document->getExtraEidValue('video')){ ?>info<?php }else{ ?>active<?php } ?> height70px">
...
echo $__Context->module_info->display_videoplayheight;
		...
		<?php if($__Context->document_list&&count($__Context->document_list))foreach($__Context->document_list as $__Context->no=>$__Context->document){ ?><tr<?php if($__Context->document_srl==$__Context->document->document_srl){ ?> class="active height70px"<?php } ?>>
			<?php if($__Context->list_config&&count($__Context->list_config))foreach($__Context->list_config as $__Context->key=>$__Context->val){ ?>
			<?php if($__Context->val->type=='no' && $__Context->val->idx==-1){ ?><td class="hidden-xs width70 text-center noBold height70px">
...
echo $__Context->no;
			...
			<td>
				<a href="<?php echo getUrl('document_srl',$__Context->document->document_srl, 'listStyle', 'list', 'cpage','') ?>" class="title custom_title paddingLeft80" >
			...
			<?php if($__Context->val->type=='regdate' && $__Context->val->idx==-1){ ?><td  class="tabledate custom_date" title="<?php echo getTimeGap($__Context->document->get('regdate'), "H:i") ?>">
				<div class="custom_ym"><?php echo $__Context->document->getRegdate('Y.m.d') ?></div>
			</td><?php } ?>
			<?php if($__Context->val->type=='last_update' && $__Context->val->idx==-1){ ?><td class="hidden-xs tabledate"><?php echo zdate($__Context->document->get('last_update'),'Y.m.d') ?></td><?php } ?>
			<?php if($__Context->val->type=='last_post' && $__Context->val->idx==-1){ ?><td class="hidden-xs tabledate">
				<?php if((int)($__Context->document->get('comment_count'))>0){ ?>
