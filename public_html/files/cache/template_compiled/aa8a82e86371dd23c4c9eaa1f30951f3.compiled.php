...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/KSO','_setting.html') ?>
...
<div class="form-inline">	
		<?php if($__Context->module_info->use_category=='Y'){ ?><select name="category_srl" class="form-control">
			...
			<option <?php if(!$__Context->val->grant){ ?>disabled="disabled"<?php } ?> value="<?php echo $__Context->val->category_srl ?>" <?php if($__Context->val->grant&&$__Context->val->selected||$__Context->val->category_srl==$__Context->oDocument->get('category_srl')||Context::get('category_srl')==$__Context->val->category_srl){ ?>selected="selected"<?php } ?>>
...
<div class=" paddingtb10">
	<div class="extra">
		<div class="input-group" style="width:100%;">
			...
			<input type="text" name="title" id="postTitle" class="text" value="<?php echo htmlspecialchars($__Context->oDocument->getTitleText()) ?>" />
			<?php }else{ ?>
			<input type="text" name="title" id="postTitle" class="text" value="<?php echo $__Context->lang->title ?>" title="<?php echo $__Context->lang->title ?>" onfocus="if(this.value==this.title) this.value='';" />
			<?php } ?>
		</div>		
	</div>	
</div>	
